...
import getWatsonXAIRecommendation from '@salesforce/apex/AgrosphereController.makeWatsonCallout';

export default class AgrosphereAiRecommendation extends LightningElement {
    @track isSpinner = false
    @track heading = 'AI RECOMMENDATION - WatsonX AI'
    ...
    get formattedText() {
        ...
        const formattedParagraphs = paragraphs.map(paragraph => {
            if (paragraph.startsWith('**')) {
                ...
                return `<h3 style="font-size: 24px;margin-top: 15px;color: #2a9d8f;font-weight: bold;">${paragraph.replace(/\*\*/g, '')}</h3>`;
    ...
    get formattedTextMobile() {
        ...
        const formattedParagraphs = paragraphs.map(paragraph => {
            if (paragraph.startsWith('**')) {
                // If it starts with '**', it's a heading
                return `<h3 style="font-size: 20px;margin-top: 15px;color: #2a9d8f;font-weight: bold;">${paragraph.replace(/\*\*/g, '')}</h3>`;
            } else if (paragraph.startsWith('*')) {
                // If it starts with '*', it's a bullet point
                const bulletPoints = paragraph.split('\n*');
